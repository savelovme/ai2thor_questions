...
def main(dataset_type):
    ...
    def create_dump():
        ...
        prefix = 'questions/'
        if not os.path.exists(prefix + dataset_type + '/data_size_compare'):
            os.makedirs(prefix + dataset_type + '/data_size_compare')

        h5 = h5py.File(prefix + dataset_type + '/data_size_compare/Size_Compare_Questions_' + time_str + '.h5', 'w')
        h5.create_dataset('questions/question', (num_record, 5), dtype=np.int32)
        print('--------------------------------------')
        print('Generating %d size_compare questions' % num_record)
        print('--------------------------------------')
        ...
        while data_ind < num_record:
            ...
            while k < num_samples_per_scene and num_tries < 10 * num_samples_per_scene:
                # randomly pick two objects in the scene
                object1_class, object2_class = random.sample(all_object_classes, 2)
                question = SizeCompareQuestion(object1_class,
                                                   object2_class)  # randomly generate a general size_compare question
